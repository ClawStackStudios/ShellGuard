...
router.post('/', requireAuth, requirePermission('canWrite'), validateBody(AttachmentSchemas.create), (req: AuthRequest, res) => {
  ...
  try {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, req.userUuid, title.trim(), file_data, file_name || '', mime_type || '', category || 'Personal', new Date().toISOString());
...
router.put('/:id', requireAuth, requirePermission('canEdit'), validateBody(AttachmentSchemas.update), (req: AuthRequest, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    db.prepare('UPDATE vault_secure_attachments SET title = ?, file_data = ?, file_name = ?, mime_type = ?, category = ? WHERE id = ? AND owner_uuid = ?')
      .run(title.trim(), file_data, file_name || '', mime_type || '', category || 'Personal', id, req.userUuid);
