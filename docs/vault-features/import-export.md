...
- **HTTP 207 Multi-Status Wire Contract**: If some items in the import payload are malformed (e.g. invalid type, missing required title/secret), the server successfully persists all valid items and returns a `207 Multi-Status` envelope with `{ inserted: string[], errors: [{ index, reason }] }`.
