...
**v0.0.1.9 (Build 18) — RELEASED & LIVE** (tag `v0.0.1.9`, GitHub Release mirrors RELEASE-v0.0.1.9.md; main synced). In-flight branch `docs/spine-chronology-reorg` (PR held): genome chronology restored (2026-09-15, Option C — spine **Stage N = Phase N−1**, Stage 18.5 = hotfix interlude, Stages 19–24 = Phases 18–23 queued; 26/26 links verified); ROADMAP reorganized to TOTP chronology (frontmatter, inline interlude, completed 15→16→17, archive 1–14); root docs truth-synced to v0.0.1.9 (ARCHITECTURE: ClawKey Method + canon, 0004/keyLedger.ts, pod purity, deltas #19/#20; SECURITY: Key Ledger Hardening narrative; BLUEPRINT: schema matches 0004); **ClawKey canon landed in UI** (14 strings ShellKey™→ClawKey™, gates green: 15 files/210 tests + build); docs/ portal truth-synced (schema ground truth → migration 0004, privacy file names, base62 alphabet, legacy Human Key alias); **bidirectional docs↔code audit: 8 docs-lies corrected, docs bow to code** (PRAGMA rekey, authLimiter 10/15m skip-success, real identity-file shape `shellguard_identity_<username>.json`, phantom `customFields.ts`, `tlsManager.ts`, real crypto exports, fifth `canMove` mask + 7 wizard presets, `_custom` AAD namespaces; claim battery zero red, portal build green); **Phase 24 queued + the lens formalized** (Cryptographic Audit Hardening & Third-Party Auditability, Tasks 47/48, provisional v0.0.2.6/Build 25 at the queue tail — Stage 25 in the spine; Documentation Impact blockquotes embedded in every queued phase with docs-hygiene as definition-of-done; Third-party-auditable standard in projectBrief; Auditability Invariants in systemPatterns; decision-log lens entry). **NEXT: Phase 18 — Unified Bitwarden-Style Item Composition & In-Browser Keypair Generation (Tasks 35/36, v0.0.2.0 / Build 19), queued & gated on Lucas's green-light**; Phases 19–23 follow, then Phase 24 closes the loop with the auditor's battery.
