...
export function FolderInputGroup({
  ...
  const [isCustomMode, setIsCustomMode] = useState(() => {
    return !["Personal", "Work", "Custom", "Test Folder", "VPN Shit", "Server Operating Systems", "Router Operating Systems"].includes(category);
  });
  const [customInput, setCustomInput] = useState(category || "Personal");

  const availablePods = useMemo(() => {
    return getAllUniquePods(items, true);
  }, [items]);
  ...
  const currentColor = getPodColor(category);
  ...
  return (
    <div className="space-y-2">
      ...
      {!isCustomMode ? (
        <select
          value={availablePods.includes(category) ? category : "__CUSTOM__"}
        ...
        >
          ...
          <option value="__CUSTOM__">+ Create Custom Pod...</option>
        </select>
      ) : (
        <div className="space-y-2">
          <div className="relative">
            <input
              ...
              placeholder="e.g. Work/Finance, Personal/Banking, Research"
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {DEFAULT_SUGGESTED_PODS.slice(0, 6).map((sug) => {
