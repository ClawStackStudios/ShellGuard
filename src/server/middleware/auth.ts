...
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  if (keyType === 'api') {
    ...
    if (!checkTokenExpiry(row.expires_at)) {
      audit.log('AUTH_FAILURE', { actor: row.owner_uuid, action: 'validate_token', outcome: 'failure', resource: 'api_token', details: { reason: 'Token expired' } });
    ...
    if (row.owner_type === 'human') {
      ...
      finalUserUuid = agent.owner_uuid;
...
export function requireHuman(req: Request, res: Response, next: NextFunction): void {
  ...
  res.status(403).json({ success: false, error: 'Forbidden: This area of the Shell requires Human identity' });
}
