...
Stores login credentials. `secret`, `totp_secret`, `attachments`, `custom_fields`, and `password_history` contain client-side ShellCryption ciphertext envelopes. `uris` is sealed under Layer 2 metadata encryption.
