...
**v0.0.2.0 "The Composite Reef" SHIPPED (2026-09-18)** — Phase 18 (Tasks 35/36, Build 20) released: tag `v0.0.2.0` → merge `5fd459e`, Release Pipeline/Docker/docs all green, release body mirrors `RELEASE-v0.0.2.0.md`. House canon: **ClawKey / ShellCryption / LobsterKeys** (internals `deriveShellKey`/`shellKey`/`ShellKeyFallback` are cross-project contracts — NEVER rename). **🚧 PHASE 19 IN-FLIGHT** (Tasks 37/38, v0.0.2.1 / **Build 22**; green-lit, branch `feat/phase19-attachment-blobs` created). Task 38 additionally carries the Eye-beside-Copy fold-in (every masked field row). Task 37 grain (mapped): `attachments.ts` stores `file_data` as base64 TEXT (client sends dataUrl, route INSERTs raw; 10MB cap via `MAX_ATTACHMENT_BYTES` in attachmentUtils + scoped 32mb body limit); zod `key`-style length-only validation; NO migration 0005 exists yet — write `migrations/0005_attachment_blobs.{up,down}.sql` (BLOB column + backfill from base64 TEXT), streaming upload handlers, 50MB/file + 500MB/owner quota (413 on breach), `tests/attachments-blob.test.ts`; SQL: use parameterized only, owner_uuid scoping mandatory. Next session: implement Task 37, then Task 38 (streams/progress/previews + ergonomics fold-in), docs 📚 impact, release v0.0.2.1. Phase 19 now includes the **Eye-beside-Copy ergonomics fold-in** (from P22/T44, Lucas 2026-09-18): Unmask toggle immediately LEFT of Copy on every masked field row (password, SSH private key, hidden custom fields), full-value mask invariant. The lens lives in `long-term/auditPerspective.md`; constraints in `long-term/constraints.md` (`.clinerules` is Cline's territory; `.agents/` is Antigravity's — no writes).
