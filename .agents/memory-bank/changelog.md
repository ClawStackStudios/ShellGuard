...
- **Phase 21 Sub-Phase: Bitwarden Ingestion Parity, Item Password History & Dual Export Suite (Sub-Phases 21.1, 21.2, 21.3)**:
  ...
  - **Dual Export Suite (`src/lib/vaultExport.ts`)**: Zero-knowledge AES-256-GCM encrypted backup envelopes (`v: 1`) sealed with ClawKey (HKDF-SHA256) or custom user passphrase (PBKDF2-SHA256, 600,000 iterations with async native WebCrypto acceleration and pure-TS fallback for non-secure origins), enforcing CSPRNG without fallback; RFC 4180 CSV spreadsheet export with toggleable password sanitization audit controls.
