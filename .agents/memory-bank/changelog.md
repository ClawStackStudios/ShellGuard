...
- **Root-docs coherence + ClawKey canon (2026-09-16)** — ARCHITECTURE/SECURITY/BLUEPRINT synced to v0.0.1.9 runtime; 14 UI strings renamed ShellKey(TM) -> ClawKey(TM); internals kept as cross-project contracts.
- **Bidirectional docs<->code audit (2026-09-16)** — 8 docs-lies corrected (PRAGMA rekey, limiter counts, identity-file shape, canMove, _custom AAD); docs bow to code.
- **Phase 24 queued + Documentation Impact crawl + the lens enters the bank (2026-09-16)** — auditor angles formalized (Tasks 47/48); docs-hygiene embedded in the schedule; Third-party-auditable standard in projectBrief.
