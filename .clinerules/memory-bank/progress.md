...
**Phase 21 (Build 25) — IMPLEMENTED & COMMITTED, pending merge + release (2026-09-20)**. Commits `6f862e5` (feat: Bulk Import Endpoint & Batch Operations, Tasks 41/42) + `3b40008` (docs sync) on `feature/phase-21-bulk-operations-11309179680338905330`. **Task 41** — `POST /api/vault/bulk-import` in `src/server/routes/vault.ts` (registered ABOVE the `:id` family after fixing the route-shadowing P0 that made the feature unreachable), scoped 10MB parser in `server.ts`, per-record `VaultSchemas.bulkImportItem.safeParse()` producing field-qualified `{index, reason}` errors, atomic `db.transaction()` insert using `prepareWrite` for the 6 metadataGuard columns with byte-for-byte pass-through for `secret`/`totp_secret`/`custom_fields`/`attachments`, 207 Multi-Status partial success; `DELETE /api/vault/bulk` (`canDelete`, transaction-wrapped attachment cascade + audit, 207 on missing IDs). **Task 42** — tri-state multi-select in `ItemListPane.tsx` with selection state lifted to `VaultShell.tsx`, floating bulk action bar (`Move to Pod` / `Assign Tag` / `Delete`) gated on `!isLocked` with Reef Modernist modals (`ConfirmDialog` + custom input modal) replacing native `prompt()`/`confirm()`, `ImportExportView.tsx` preview table + "Skipped Items (N)" error-resolution chips fed by the 207 `data.errors[]`. **Witness test** — `tests/vault-bulk-import.test.ts` (350 lines, 11 tests, port 64650) proves the headline criterion (**100 items / 2 malformed → 207, 98 persisted**), ciphertext opacity roundtrip, tenant isolation, permission gates on both routes, and cascade atomicity. **Verified gates**: 21 test files / **259 passed / 1 skipped / 0 failed**, `tsc --noEmit`, `vite build` (2177 modules, 55s), `docs:build` (~101s). **NEXT: merge to `main` → cut v0.0.2.3 (Build 25) → Phase 22.**
