...
└── src/
    ...
    │   │   ├── attachmentBlobs.ts     #   Phase 19 in-code attachment BLOB backfill & quota migration
...
│   │                              #   hashToken (SHA-256), identity-file download
    ...
    │   ├── keyGen.ts                  #   In-browser Ed25519 & RSA-4096 SSH keypair generator
    │   ├── attachmentUtils.ts         #   Multipart upload streaming, progress tracking, MIME validation
...
> Prior to Phase 13 (v0.0.1.3) there were **no admin endpoints**. The SuperLobster Panel (v0.0.1.3) ships with its own secrets-aware threat model in [ADMIN.md](./ADMIN.md) — strict metadata, whitelist settings, no downloads, no HTTP restore.
