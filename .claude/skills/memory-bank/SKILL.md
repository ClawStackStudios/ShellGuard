---
name: memory-bank
description: Claude's Memory Bank system for maintaining project knowledge across sessions. Use this skill at the start of every task to read all memory bank files and understand the project context. Also use when the user asks to "update memory bank", "save progress", "document what we did", or after significant changes that should be captured. This is Claude's only link to previous work after memory resets — it must be maintained with precision.
---

# Claude's Memory Bank (Time-Aware Version)
